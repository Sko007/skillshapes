...
import { Button, ButtonDropdown, DropdownToggle, DropdownMenu, DropdownItem,UncontrolledButtonDropdown, Modal, ModalHeader, ModalBody, ModalFooter, Input, Label, Form, FormGroup } from "reactstrap"
  ...
  const handleSelectChange = (e) => {
      ...
      useEffect(() => {
        const timeout = setTimeout(() => {
          ...
          const svg = d3.select('#skillshape').attr('width', WIDTH + margin.left + margin.right).style("z-index", 1);
        ...
        <img className="image-small" src="content/images/default_bild.jpg" alt="Logo" />
          ...
          <div>
            <ul className="list" style={{ fontSize: '24px', display: 'block' }}>
              ...
              <Button color="secondary" size="lg" style={{ backgroundColor: color ? '#f7485e' : '', width: '100%' }} onClick={Level1} block>Übersicht</Button>
              <hr></hr>
              {skillshapeData[2].map((item: any, i: number) => {
                return (
                  ...
                  id={'button' + i}
                  style={{ width: '100%' }}
                  onClick={(): void => Level2(i, item.id)} color="secondary" size="lg" block> {item.name}
                  </Button>
                );
              })}
            </ul>
          ...
          <Form inline onSubmit={(e) => e.preventDefault()} >
                <FormGroup>
                    <Input className="mx-2" type="select" id="focusAfterClose" onChange={e =>handleSelectChange(e)} >
                       <option ref={optionRef} value="null" >Bitte wählen</option>
                        <option ref={optionRef1}  value="true"  >Nach Alphabet sortieren</option>
                        <option ref={optionRef2}  value="false"  >Nach Können sortieren</option>
                    </Input>
                </FormGroup>
            </Form>
